...
static cl::opt<bool> EnableParallelForRegionMerging(
"openmp-opt-enable-for-loop-merging", cl::ZeroOrMore,
cl::desc("Enable the OpenMP for loop region merging optimization."), cl::Hidden,
cl::init(false));
...
struct OpenMPOpt {
  ...
  bool run() {
    ...
    if (EnableParallelRegionMerging) {
     ...
     /// Currently, the optimization will be executed as part of the -O3 as default optimization for OpenMP optimization pipeline
    Changed |= mergeOMPForLoopRegions();
...
private:
    /// The Optimization will merge the openmp for loops if they have same schedulining parameters or it is safe to merge
    /// TODO: Conditional omp for loop merging when its is (under development)
    bool mergeOMPForLoopRegions(){
        OMPInformationCache::RuntimeFunctionInfo &RFI =
            OMPInfoCache.RFIs[OMPRTL___kmpc_for_static_init_4];
        
        if (!RFI.Declaration)
            return false;
        
        //bool Changed = false;
        SmallVector<CallInst *,8> callInst;
        SmallVector<BasicBlock*,8> basicblock;
        MapVector<Value*, Value*> store_op0_op1, args_map;
        MapVector<CallInst*, SmallVector<CallInst *,8>> mapper;
        MapVector<CallInst*, CallInst*> MapInitFinCI;
        
        /// It takes a call instructions and extract arguments that needs to be compared
        auto BuildArguments = [&] (SmallVector<Value*, 8>& A, CallInst& I){
            A.clear();
            for (Value* v: I.args())
                A.push_back(v);
        };
        /// It takes two call instructions and compares for fusion
        auto CompareTwoInstArgs = [&] (SmallVector<Value*, 8>& A1, SmallVector<Value*, 8>& A2){
            for (auto i = A1.begin(), j = A2.begin(); i != A1.end() && j!= A2.end(); i++, j++){
                if (isa<Constant>(*i) && isa<Constant>(*j)){
                    if ( *i != *j) return false;}
                else if (store_op0_op1.find(*i)->second != store_op0_op1.find(*j)->second)
                    return false;}
            for (auto i = A1.begin(), j = A2.begin(); i != A1.end() && j!= A2.end(); i++, j++)
                args_map.insert({*j, *i});
            return true;
        ...
        auto CheckTheCompatibility = [&](){
            SmallVector<Value *, 8> Args1, Args2;
            SmallVector<CallInst *, 8> CompInst;
            for (auto i= callInst.begin(); i != callInst.end(); ++i){
                CompInst.clear();
                BuildArguments(Args1, **i);
                for ( auto j = i+1; j != callInst.end(); ++j){
                    BuildArguments (Args2, **j);
                    if (CompareTwoInstArgs(Args1, Args2)) {
                        CompInst.push_back(*j);
                        callInst.erase(j);j--;
                    } else {
                        if (CompInst.size()) mapper.insert({*i, CompInst});
                        break;}
                }
                if (i==callInst.end()-1 && CompInst.size()) mapper.insert({*i, CompInst});
            }
            return mapper.size();
        };
        
        auto CleanInstructions=[&](){
            for ( auto itr :mapper){
                int count = (itr.second).size();
                Instruction *I = MapInitFinCI.find(itr.first)->second;
                I->eraseFromParent();
                for (auto itr1 : itr.second){
                    Instruction *I1 = itr1;
                    Instruction *I2 = MapInitFinCI.find(itr1)->second;
                    I1->eraseFromParent();
                    if ( count==1) break;
                    I2->eraseFromParent();
                    count--;}
        ...
        auto Replace_UseValues = [&](){
            SmallVector<Instruction *,8> removeInst, removeIInst;
            for ( auto b: basicblock)
                for (BasicBlock::iterator II = b->begin(); II != b->end(); ++II) {
                    if (IntrinsicInst *I = dyn_cast<IntrinsicInst> (II)){
                        if (I->getIntrinsicID() == Intrinsic::lifetime_start || I->getIntrinsicID() == Intrinsic::lifetime_end )
                            removeIInst.push_back(I);continue;}
                    Instruction *It = dyn_cast<Instruction>(II);
                    if (isa<CallInst>(It)) continue;
                    for (unsigned int k = 0; k < It->getNumOperands(); k++){
                        auto temp =  args_map.find(It->getOperand(k));
                        if (temp != args_map.end()){
                            It->setOperand(k, temp->second);
                            if (isa<StoreInst>(It) && k > 0) removeInst.push_back(It);
                        }
                }
            }
            for (auto r: removeInst)
                    r->eraseFromParent();
            for (auto r: removeIInst)
                    r->eraseFromParent();
        };
        /// Cleaning the Instrinsic Instructions
        auto CleanIntrinsicCalls=[&](){
